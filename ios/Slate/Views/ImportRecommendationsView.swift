        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    Text("FROM ANYWHERE")
                        .font(.caption.monospaced().weight(.semibold)).tracking(1.4).foregroundStyle(.secondary)
                    Text("Bring recommendations into slate")
                        .font(.system(size: 36, weight: .bold)).tracking(-1.4)
                        .padding(.top, 5)
                    Text("Share or paste an Instagram, TikTok, YouTube, IMDb, Letterboxd, or article link. Slate finds the films and shows inside it.")
                    ...
                    TextEditor(text: $text)
                        ...
                        .padding(12)
                        .background(.white.opacity(0.05), in: .rect(cornerRadius: 17))
                        .overlay { RoundedRectangle(cornerRadius: 17).stroke(.white.opacity(0.1), lineWidth: 0.7) }
                        .padding(.top, 25)
                        .overlay(alignment: .topLeading) {
                            if text.isEmpty {
                                Text("Paste a link or recommendation text")
                                    .foregroundStyle(.tertiary).padding(.top, 40).padding(.leading, 17)
